...
impl OutputZoomState {
    ...
    pub fn zoomed_geometry(&self) -> Option<Rectangle<i32, Global>> {
        let output_geometry = self.output.geometry();
        let focal_point = self.current_focal_point().to_global(&self.output);
    ...
    pub fn update_focal_point(
        ...
        let output_geometry = self.output.geometry().to_local(&self.output).to_f64();
        let zoomed_output_geometry = self
            .zoomed_geometry()
            .unwrap()
            .to_local(&self.output)
            .to_f64();
        ...
        match movement {
            ZoomMovement::Continuously => self.focal_point = cursor_position,
            ZoomMovement::OnEdge => {
            ...
            ZoomMovement::Centered => {
                ...
                let level = level.unwrap_or(self.animating_level());
    ...
    pub fn update(&mut self, level: f64, animate: bool, increment: u32) {
        tracing::warn!("update level = {level}, animate = {animate}, increment = {increment}");
    ...
    fn render<R, C>(&mut self, renderer: &mut R) -> Vec<C>
    where
        ...
        R::TextureId: Send + Clone + 'static,
    {
        ...
        let output_geo = self.output.geometry().to_f64();
...
impl ZoomState {
    ...
    pub fn update_focal_point(
        ...
        output_state
            .lock()
            .unwrap()
            .update_focal_point(cursor_position, level, self.movement);
    }
    ...
    {
        let output_state = output.user_data().get::<Mutex<OutputZoomState>>().unwrap();
        output_state.lock().unwrap().render(renderer)
    }
}
