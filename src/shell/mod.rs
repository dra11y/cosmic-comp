...
impl Common {
    pub fn add_output(&mut self, output: &Output) {
        ...
        if shell.zoom_state.is_some() {
            let config = self.config.cosmic_conf.accessibility_zoom;
    ...
    pub fn update_config(&mut self) {
        ...
        if let Some(zoom_state) = shell_ref.zoom_state.as_mut() {
            let zoom_config = &self.config.cosmic_conf.accessibility_zoom;
            zoom_state.show_overlay = zoom_config.show_overlay;
            ...
            for output in shell_ref.workspaces.sets.keys() {
                ...
                output_state
                    .lock()
                    .unwrap()
                    .update_config(output, zoom_config);
            }
        }
...
impl Shell {
    ...
    pub fn update_pointer_position(&mut self, location: Point<f64, Local>, output: &Output) {
        ...
        if let Some(zoom_state) = self.zoom_state.as_mut() {
            zoom_state.update_pointer_position(output, location);
        }
    }
