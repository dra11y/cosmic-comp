...
impl Common {
        ...
        if shell.zoom_state.is_some() {
            output.user_data().insert_if_missing_threadsafe(|| {
                Mutex::new(OutputZoomState::new(
                    output,
                    &self.config.cosmic_conf.accessibility_zoom,
                    1.0,
    ...
    pub fn update_config(&mut self) {
        ...
        if shell_ref.zoom_state.is_some() {
...
impl Shell {
    ...
    pub fn trigger_zoom(
        ...
        self.zoom_state = Some(ZoomState::new(seat.clone()));
    }
            ...
            if o == output {
                ...
                for (i, workspace) in set.workspaces.iter_mut().enumerate() {
                    if i == set.active {
                        ...
                        if let Some(zoom_state) = self.zoom_state.as_ref() {
                            zoom_state
                                .output_state(output)
                                .lock()
                                .unwrap()
